...
function makeImportServiceSpy(
	validationResult: ImportValidationResult = { valid: true, sessionCount: 2, exerciseCount: 3 }
) {
...
describe("ImportDataUseCase", () => {
	...
	describe("validate()", () => {
		...
		it("should set importCount signal and clear importError on successful validation", async () => {
			importServiceSpy.validate.and.returnValue(Promise.resolve({ valid: true, sessionCount: 2, exerciseCount: 5 }));
