...
function makeProviders(activeLang = signal<'fr' | 'en'>('fr'), activeMassUnit = signal<'metric' | 'imperial' | 'us'>('metric')) {
  ...
  const massUnitServiceSpy = { activeMassUnit, setMassUnit: jasmine.createSpy('setMassUnit'), detect: jasmine.createSpy('detect').and.returnValue('metric') };
...
describe('SessionListComponent — sélecteur de langue', () => {
  ...
  beforeEach(() => {
    const { getSessionsSpy, createSessionSpy, duplicateSessionSpy, deleteSessionSpy, setLanguageSpy, languageServiceSpy, massUnitServiceSpy, routerSpy, requestReviewSpy, reviewServiceSpy } = makeProviders();
...
describe('SessionListComponent — sélecteur d\'unité de masse', () => {
  let fixture: ReturnType<typeof TestBed.createComponent<SessionListComponent>>;
  let massUnitServiceSpy: { activeMassUnit: ReturnType<typeof signal<'metric' | 'imperial' | 'us'>>; setMassUnit: jasmine.Spy; detect: jasmine.Spy };
