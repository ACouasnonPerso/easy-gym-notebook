...
export class CardioTimeChartComponent {
	...
	readonly chartData = computed(() => {
		...
		const timePolyline = n > 1 ? smoothPath(timePoints) : "";
		const timeAreaPoints = n > 1 ? smoothPath(timePoints) + ` L ${xCoords[n - 1]},110 L ${xCoords[0]},110 Z` : "";
	...
	readonly regressionOverlay = computed((): RegressionOverlay => {
		...
		return computeRegressionOverlay(cd.timePoints, this.regressionService, cd.durations, this.groupBy());
	});
}
